...
def _capability_template(
    ...
    *,
    ...
    cols = columns or {}
    template_domain = _template_domain_for(capability, domain)
...
def _template_code_for(
    ...
    *,
    ...
    if domain == "perturbseq":
        from pertura.domain.perturbseq_templates import template_code_for
        return template_code_for(
            capability_id,
            mode=mode,
            target=target,
            columns=columns,
...
def _template_domain_for(capability: dict, domain: str) -> str:
    if domain == "perturbseq":
        return "perturbseq"
    functions = [str(item) for item in capability.get("functions", []) or []]
    packages = {str(item) for item in capability.get("packages", []) or []}
    observations = {str(item) for item in capability.get("expected_observations", []) or []}
    if (
        "scanpy" in packages
        and any(item.startswith("scanpy.") for item in functions)
        and observations & {"de_result", "qc_metric", "target_coverage", "guide_assignment", "embedding_summary"}
    ):
        return "perturbseq"
    return domain
