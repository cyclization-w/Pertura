"""Independent, package-safe smoke checks for Pertura core claims."""
...
from pertura import AnalysisGraph, CapabilityRegistry, capability, conditions as c, audit_analysis_graph
...
CLAIMS = {
    "analysis_graph": {
        "title": CORE_CLAIMS["analysis_graph"]["title"],
        "fn": "claim_analysis_graph_and_gate",
    },
    "observation_memory": {
        "title": CORE_CLAIMS["observation_memory"]["title"],
        "fn": "claim_observation_memory",
    },
    "deliberative_audit": {
        "title": CORE_CLAIMS["deliberative_audit"]["title"],
        "fn": "claim_deliberative_audit_and_evidence",
    },
}
...
class _WorkbenchStub:
    def __init__(self, controller: GraphController, store: Store, run_id: str):
    ...
    def _emit(self, event_type: str, payload: dict):
        return self._controller.append_event(event_type, payload)
...
def claim_analysis_graph_and_gate(tmp_root: Path) -> None:
    ...
    controller.append_event("node_entered", {"node_id": "inspect", "branch_id": "main", "reason": "start"})
    result = gated_dispatch(
        _WorkbenchStub(controller, store, "claim_graph"),
...
def claim_observation_memory(tmp_root: Path) -> None:
    ...
    memory = build_observation_memory_view(snap, target="TargetM", metric="logFC")
    context = build_context_view(snap, max_items=4)
    tool_view = execute_tool("query_observation_memory", {"target": "TargetM", "metric": "logFC"}, snap=snap)
...
def claim_deliberative_audit_and_evidence(tmp_root: Path) -> None:
    ...
    controller.append_event("node_entered", {"node_id": "effect", "branch_id": "main", "reason": "start"})
    wb_stub = _WorkbenchStub(controller, store, "claim_evidence")
    blocked = gated_dispatch(
        wb_stub,
        "execute_code",
        "print('explore')",
    ...
    check("missing capability declaration is blocked", blocked == "blocked")
    planned = gated_dispatch(
        wb_stub,
        "execute_code",
        "print('effect')",
        {"summary": "effect run"},
        {"capability_ids": ["run_de"], "stage": "effect"},
        store.read_snapshot(),
    )
    check("declared allowed capability can commit", planned == "planned_attempt")
    ...
    rethinking = plan_rethinking(snap, "con_claim", issue="verify before reporting", graph=graph)
    tool_review = execute_tool("review_evidence_chain", {"node_id": "con_claim"}, snap=snap)
    tool_rethinking = execute_tool("plan_rethinking", {"node_id": "con_claim", "issue": "verify before reporting"}, snap=snap)
    ...
    check("rethinking plan exposes trace loop", rethinking["view_type"] == "rethinking_plan")
    check("rethinking plan has action menu", any(item.get("tool") == "review_evidence_chain" for item in rethinking["recommended_actions"]))
    check("LLM can call evidence review tool", tool_review["view_type"] == "evidence_chain_review")
    check("LLM can call rethinking tool", tool_rethinking["view_type"] == "rethinking_plan")
    ...
    check("audit toolbox points to rethinking", "plan_rethinking" in toolbox_tools)
...
def _result_payload(selected: list[str]) -> dict[str, object]:
    ...
    return {
        ...
        "ok": FAIL == 0,
        "summary": {"passed": PASS, "failed": FAIL, "total": PASS + FAIL},
        "claims": claim_results,
    }


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run independent Pertura paper-claim smoke checks.")
    parser.add_argument("--list-claims", action="store_true", help="List runnable paper claims.")
    parser.add_argument("--json", action="store_true", help="Emit machine-readable JSON only.")
    parser.add_argument("--claim", action="append", default=[], help="Claim id to run. May be repeated or comma-separated.")
...
def main() -> int:
    ...
    if args.list_claims:
        payload = {
            "test_type": "paper_claim_segments",
            "claims": [{"claim_id": claim_id, "title": payload["title"]} for claim_id, payload in CLAIMS.items()],
        }
        if args.json:
            print(json.dumps(payload, indent=2))
        else:
            for item in payload["claims"]:
                print(f"{item['claim_id']}: {item['title']}")
        return 0
    payload = run_claims(args.claim, json_output=args.json)
    if args.json:
        print(json.dumps(payload, indent=2))
    else:
