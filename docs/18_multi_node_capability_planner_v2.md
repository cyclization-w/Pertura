...
This document defines a post-benchmark target architecture. The earlier a19 P0
execution-brief, CodeAct-handoff, and completion-guard experiment was a pilot
and is not part of the final paper runner. It is not a claim that Pertura
already performs general autonomous scientific planning.
...
definitions, dataset splits, evaluators, and scientific environments. The
final paper path uses only answer-free static capability-contract subsets and
frozen task-to-skill bindings; it does not compile a dynamic plan or active
window.
...
window after every committed result. No such compiler runs in the final a19
paper benchmark.
...
## Retired a19 P0 pilot

An early a19 pilot omitted the natural-language Intent Adapter and same-turn
dynamic replanning. It treated `expected_capability_dag` as a frozen candidate
allowlist and compiled a task execution brief. Live canaries showed that this
overlapped with skill instructions, CodeAct ownership, provider finalization,
and the runtime's existing dependency/promotion machinery. The pilot is
retained in source and tests for post-benchmark research, but is disabled in
the formal paper runner.
...
The final paper runner instead writes only
`task/capability_contracts/<task_id>.json`: an answer-free static subset of the
bound capability catalog. It contains no route, node status, active window,
completion checklist, reference, threshold, or evaluation truth.
...
CodeAct does not mean unconstrained package discovery. Paper tasks that require
it bind a static `codeact_protocol` with analysis-unit, design, pairing,
baseline/target, column, and input-role fields. The formal runner serializes
that catalog field directly and does not instantiate a handoff state machine.

This binding is deliberately narrower than the full V2 Intent Adapter. It does
not claim to choose a protocol for arbitrary new data. A missing or unready
frozen protocol is reported rather than guessed. General intent resolution,
role inference, and dynamic protocol selection remain post-benchmark work.

The static task context tells `pertura_full` that dataset inspection is already
complete, supplies registered asset IDs and capability contracts, and prohibits
source/YAML/test/environment-directory exploration for contract discovery.

## Retired completion-guard pilot

The early pilot tested a shared 24-call closure guard:
...
This guard is disabled in the formal benchmark because it created a second
owner for task finalization. The final runner instead preinitializes a neutral
blocked result, records its hash, and deterministically fails any task whose
provider does not update it. It never fabricates, moves, repairs, or
scientifically completes missing results.
...
- PAPA-06 and PAPA-07 receive their frozen protocol/evidence context without a
  compiled route;
- no capability-contract subset or bundled skill reaches either baseline;
- the neutral result checkpoint and update gate behave identically across
  conditions;
