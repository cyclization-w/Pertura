...
a scientific fallback bind a frozen generic `codeact_protocol_id` plus
structured analysis-unit, design, pairing, and input-role fields. The compiler
instantiates an answer-free `pertura-codeact-handoff-v1` from that binding, the
registered assets, the task output contract, and the locked environment
manifest. The handoff names the environment variable, script destination,
invocation command, outputs, and authority boundary. It is canonical-hashed;
it is never generated by the provider or selected by substring matching.
...
discovery. A ready CodeAct handoff likewise prohibits probing rpy2, alternate
package managers, or unrelated environments before using its locked runner.
