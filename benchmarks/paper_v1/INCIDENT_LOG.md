...
| PB-064 | REF-04 selected the first `mixscape_class*` Pertpy column, which could be a probability column rather than the exact categorical `mixscape_class` output. Strict evaluator qualification exposed numeric values being treated as class labels. | Require the exact categorical output column, reject prefixed score/probability columns, regenerate REF-04, and invalidate all bindings to the affected reference hashes. | The superseded REF-04 and checkpoints bound to it are not valid paper evidence. | `verified` |
