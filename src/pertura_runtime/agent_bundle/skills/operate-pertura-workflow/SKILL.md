...
2. In registered-contract mode, invoke every listed `invocation_binding` exactly once, in order, using its `minimal_call`. The binding already freezes the capability, DatasetContract, scope, assets, dependencies, parameters, hashes, and output mapping. Do not copy those fields into a hand-written call or replace them with paths. Treat a genuine scientific `blocked` or `unresolved` response about missing observed data, unresolved design identity, incompatible scope, or missing independent replicates as information to preserve, not as permission to guess.
